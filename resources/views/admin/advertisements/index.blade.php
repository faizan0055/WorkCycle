            ...
            <div class="col-lg-12 col-xl-12 col-md-12 col-sm-12">
                <div class="card  box-shadow-0">
                    <div class="card-header">
                        <h4 class="card-title mb-1">ADD ADVERTISEMENT</h4>
                    </div>
                    <div class="card-body pt-0">
                        <form class="form-horizontal needs-validation" method="post" action="{{ isset($advertisement) ? route('advertisements.update',$advertisement->id) : route('advertisements.store') }}" enctype="multipart/form-data">
                            @csrf
                            ...
                            <div class="row">
                                <div class="col-4">
                                    <div class="form-group">
                                        ...
                                        <input type="text" class="form-control {{ $errors->has('name') ? ' is-invalid' : '' }}" name="name" value="{{ isset($advertisement) ? $advertisement->name : '' }}" id="name" placeholder="Name">
                                        @if($errors->has('name'))
                            ...
                            <div class="form-group mb-0 justify-content-end mt-2">
                                ...
                                <textarea class="form-control textarea1 {{ $errors->has('detail') ? ' is-invalid' : '' }}" name="detail">{{ isset($advertisement) ? $advertisement->detail : '' }}</textarea>
                            ...
                            <div class="form-group mb-0 mt-3 justify-content-end">
                                <div>
                                    <button type="submit" class="btn btn-danger">{{ isset($advertisement) ? 'Update Now' : 'Save Now' }}</button>
                                </div>
                            </div>
                        </form>
                    </div>
                    ...
                    <div class="card">
                        <div class="card-header pb-0">
                            <div class="d-flex justify-content-between">
                                <h4 class="card-title mg-b-0">ADVERTISEMENT LIST</h4>
                        ...
                        <div class="card-body">
                            <div class="table-responsive">
                                <table class="table mg-b-0 text-md-nowrap">
                                    ...
                                    <tbody>
                                    @foreach($advertisements as $index => $add)
                                        <tr>
                                            <th scope="row">{{++$index}}</th>
                                            <td><img src="{{url('images/advertisement',$add->image)}}" width="50"></td>
                                            <td>{{$add->name}}</td>
                                            <td>{{$add->detail}}</td>
                                            <td><a class="btn btn-sm btn-info" href="{{route('advertisements.edit',$add->id)}}"><i class="fa fa-edit"></i></a>
                                                <a class="btn btn-sm btn-warning"  href="javascript:void(0);" onclick="$(this).find('form').submit();"><i class="fa fa-trash"></i>
                                                    <form action="{{ route('advertisements.destroy', $add->id) }}"
                                                          method="post"
